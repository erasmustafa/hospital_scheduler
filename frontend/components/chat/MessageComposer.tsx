...
import { AtSign, MessageSquarePlus, Paperclip, Plus, Send, Smile } from "lucide-react";
...
function ComposerButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="flex h-8 w-8 items-center justify-center rounded-xl bg-white/80 text-slate-500 ring-1 ring-slate-200 transition hover:bg-blue-50 hover:text-blue-700 active:scale-95"
    >
      {icon}
    </button>
  );
  ...
  return (
    <footer className="shrink-0 border-t border-slate-200 bg-white px-6 py-3">
      <div className="mx-auto max-w-5xl">
        <div className="relative rounded-[22px] border border-slate-200 bg-gradient-to-r from-white to-[#F8FAFF] px-2.5 py-2 shadow-[0_14px_30px_-28px_rgba(15,23,42,0.24)]">
          {commandSuggestions.length > 0 || parsed.isCommand ? (
            <div className="absolute bottom-[calc(100%+10px)] left-0 right-0 rounded-2xl border border-slate-200 bg-white/95 p-2 shadow-xl shadow-slate-300/30 backdrop-blur-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {commandSuggestions.map((command) => (
                    <button
                      ...
                      className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700 transition hover:bg-blue-100 active:scale-95"
                    >
                      {command}
                    </button>
                  ))}
              ...
              {parsed.isCommand ? (
                ...
                >
                  Komut algılandı: /{parsed.command}
                  {parsed.args ? ` · ${parsed.args}` : " · içerik bekleniyor"}
                </div>
              ) : null}
            </div>
          ) : null}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <ComposerButton icon={<AtSign className="h-3.5 w-3.5" />} label="Kişi etiketle" />
              <button
                type="button"
                onClick={onOpenCommandPalette}
                className="rounded-xl bg-white/80 px-3 py-1 text-xs font-bold text-slate-700 ring-1 ring-slate-200 transition hover:bg-blue-50 hover:text-blue-700 active:scale-95"
              >
                /
              </button>
            </div>
            ...
            <ComposerButton icon={<Smile className="h-3.5 w-3.5" />} label="Emoji" />

            <button
              type="button"
              onClick={submit}
              aria-label="Gönder"
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[15px] bg-gradient-to-br from-blue-600 to-violet-600 text-white shadow-[0_18px_40px_-24px_rgba(79,70,229,0.8)] transition hover:-translate-y-0.5 hover:shadow-[0_20px_44px_-22px_rgba(79,70,229,0.9)] active:scale-95"
            >
